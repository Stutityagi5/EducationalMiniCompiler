export default function SyntaxErrors({ syntaxErrors }) {
  return (
    <section className="rounded-lg border border-slate-800 bg-slate-900 p-4">
      <div className="mb-3 text-sm font-semibold text-slate-300">Syntax Errors</div>
      <div className="h-[180px] overflow-auto rounded-md border border-slate-800 bg-slate-950 p-4 text-xs text-slate-200">
        {syntaxErrors.length === 0 ? (
          <span className="text-slate-500">No syntax errors.</span>
        ) : (
